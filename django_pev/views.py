...
class IndexesView(TemplateView):
    ...
    def get_context_data(self, **kwargs: Any) -> dict[str, Any]:
        ...
        ctx['unused_indexes'] = sorted([i for i in all_indexes if i.is_unused], key=lambda i: i.size_bytes, reverse=True)
        ctx['duplicated_indexes'] = sorted([i for i in all_indexes if i.is_duplicated], key=lambda i: i.size_bytes, reverse=True)
